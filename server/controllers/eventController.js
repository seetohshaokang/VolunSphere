	...
	try {
		...
		if (isEventRecurring) {
			...
			if (recurrence_start_date) {
				...
				return res.status(400).json({
					message:
						"Recurrence start date is required for recurring events",
				});
			}
			...
			try {
				const startDateStr =
					start_date +
					(start_time ? `T${start_time}:00` : "T00:00:00");
	...
	try {
		...
		if (existingRegistration) {
			if (
				...
				return res.status(403).json({
					...
					wasRemoved: true,
					removalReason:
						existingRegistration.removal_reason ||
		...
		if (event.is_recurring) {
			...
			if (relatedEvents.length > 0) {
				...
				const existingRecurringRegistration =
		...
		if (!volunteer.nric_image.verified) {
			return res.status(403).json({
				message:
					"You need to verify your NRIC before signing up for events",
...
exports.removeEventSignup = async (req, res) => {
	try {
		...
		console.log(
			`Event ${id} updated, new registered count: ${
				event.registered_count - 1
			}`
		);
...
exports.reportEvent = async (req, res) => {
	try {
		...
		// If user is a volunteer, check verification status
		if (user.role === "volunteer") {
			...
			if (!volunteer.nric_image.verified) {
				return res.status(403).json({
					message:
						"You need to verify your NRIC before reporting events",
					requiresVerification: true,
				});
			}
		}
...
exports.getEventVolunteers = async (req, res) => {
	try {
		...
		if (event.organiser_id.toString() !== organiser._id.toString()) {
			return res.status(403).json({
				message:
					"You are not authorized to view volunteers for this event",
			});
		}

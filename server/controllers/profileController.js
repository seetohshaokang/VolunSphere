...
exports.fetchProfile = async (req, res) => {
	try {
		...
		if (!user) {
			console.log(
				"❌ Error: User not found in database with ID:",
				userId
			);
	...
	try {
		...
		if (!user || user.role !== "volunteer") {
			return res.status(403).json({
				message:
					"Access denied. Only volunteers can upload NRIC images.",
			});
		}
			...
			events = validRegistrations
				.map((reg) => {
					...
					if (!reg.event_id || !reg.event_id._doc) {
						console.log(
							"❌ Missing event data for registration:",
							reg._id
						);
						return null;
					}
					...
					return {
						...
						registration_date:
							reg.signup_date || reg.registration_date,
						organizer: organizerData
							? {
									id: organizerData._id,
									name: organizerData.name,
									profile_picture_url:
										organizerData.profile_picture_url,
									verification_status:

...
const AdminUsers = () => {
  ...
  const getRoleBadge = (role) => {
    switch (role) {
      ...
      case 'admin':
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-800">Admin</span>;
      default:
                        ...
                        <div className="flex-shrink-0 h-10 w-10">
                          {user.profile?.profile_picture_url ? (
                            ...
                            <div className="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center text-gray-600">
                              {(user.profile?.name?.charAt(0) || user.profile?.organisation_name?.charAt(0) || user.email.charAt(0)).toUpperCase()}
                            </div>
                          )}
                        </div>
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">
                            {user.profile?.name || user.profile?.organisation_name || 'Unknown'}
                          </div>

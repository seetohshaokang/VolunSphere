...
const AdminReportDetail = () => {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'dismissed':
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-800">Dismissed</span>;
      default:
        ...
        <div className="bg-white rounded-lg shadow p-6 col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-medium">
                {report.reported_id?.name || report.reported_id?.organisation_name || 'Unknown'}
              </p>
            </div>
            {report.event_id && (
              <div>
          ...
          {report.admin_notes && (
            <div className="mt-4">
              ...
              <p className="mt-2 p-3 bg-gray-50 rounded">{report.admin_notes}</p>
            </div>
          )}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="font-medium">{report.reporter_id?.email || 'Unknown'}</p>
            </div>
            <div>
            ...
            {report.status === 'resolved' || report.status === 'dismissed' ? (
              <>
                <div>
                  ...
                  <p className="font-medium">{report.resolved_by?.name || 'Unknown Admin'}</p>
                </div>
                <div>
                  <p className="text-gray-600">Resolution Date:</p>
                  <p className="font-medium">{formatDate(report.resolution_date)}</p>
                </div>
              </>
            ) : (
              <p className="text-yellow-600">This report is still pending resolution</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
              <div>
                ...
                <p className="font-medium">{report.reported_id.name || 'Unknown'}</p>
              </div>
              <div>
                <p className="text-gray-600">Contact:</p>
                <p className="font-medium">{report.reported_id.phone || 'Not provided'}</p>
              </div>
              <div>
                <Link 
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
              <div>
                ...
                <p className="font-medium">{report.reported_id.organisation_name || 'Unknown'}</p>
              </div>
              <div>
                <p className="text-gray-600">Contact:</p>
                <p className="font-medium">{report.reported_id.phone || 'Not provided'}</p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
              ...
              <div>
                ...
                <p className="font-medium">{report.reported_id.location || 'Not provided'}</p>
              </div>
              <div>
                <p className="text-gray-600">Status:</p>
              ...
              <div>
                ...
                <p className="font-medium">
                  {formatDate(report.reported_id.start_datetime || report.reported_id.recurrence_start_date)}
                </p>
              </div>
              <div>
                <Link 
      ...
      {showStatusModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg leading-6 font-medium text-gray-900 text-center">Update Report Status</h3>
              <div className="mt-4 px-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={updateLoading || !newStatus || newStatus === report.status}
                  >

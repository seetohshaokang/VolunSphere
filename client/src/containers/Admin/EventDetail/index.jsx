...
const AdminEventDetail = () => {
    ...
    const handleStatusUpdate = async () => {
        ...
        try {
            ...
            const response = await Api.updateEventStatus(id, newStatus, statusReason);
            const data = await response.json();
    ...
    return (
        <div className="container mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-lg shadow p-6 col-span-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p className="font-medium">{event.is_recurring ? 'Recurring' : 'One-time'}</p>
                        </div>
                        {event.is_recurring ? (
                            <>
                                <div>
                                    <p className="text-gray-600">Recurrence Start:</p>
                                    <p className="font-medium">{formatDate(event.recurrence_start_date)}</p>
                                </div>
                                <div>
                                    <p className="text-gray-600">Recurrence End:</p>
                                    <p className="font-medium">{formatDate(event.recurrence_end_date)}</p>
                                </div>
                                <div>
                                    <p className="text-gray-600">Recurrence Pattern:</p>
                                    <p className="font-medium">{event.recurrence_pattern}</p>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium">{formatDate(event.end_datetime)}</p>
                                </div>
                            </>
                        )}
                        <div>
                            <p className="text-gray-600">Max Volunteers:</p>
                            <p className="font-medium">{event.max_volunteers || 'Unlimited'}</p>
                        </div>
                        <div>
                            <p className="text-gray-600">Registered Volunteers:</p>
                            <p className="font-medium">{event.registered_count || 0}</p>
                        </div>
                    </div>

                    <div className="mt-6">
                        <p className="text-gray-600">Description:</p>
                        <p className="mt-2 p-3 bg-gray-50 rounded">{event.description}</p>
                    </div>
                    ...
                    <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                            {event.causes && event.causes.length > 0
                                ...
                                : <span className="text-gray-500">No causes specified</span>}
                        </div>
                    </div>
                </div>
                        ...
                        {event.organiser_id ? (
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="font-medium">{event.organiser_id.organisation_name}</p>
                                </div>
                                {event.organiser_id.user_id && (
                                    <div>
                                        <p className="text-gray-600">Email:</p>
                                        <p className="font-medium">{event.organiser_id.user_id.email}</p>
                                    </div>
                                )}
                                <div>
                                    <p className="text-gray-600">Phone:</p>
                                    <p className="font-medium">{event.organiser_id.phone || 'Not provided'}</p>
                                </div>
                                <div>
                                    <p className="text-gray-600">Verification Status:</p>
                                    <p className="font-medium">
                                        {event.organiser_id.verification_status === 'verified'
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
                <h2 className="text-xl font-semibold mb-4">Registrations ({registrations.length})</h2>
                                ...
                                {registrations.map((reg) => (
                                    <tr key={reg._id}>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {reg.volunteer_id?.name || 'Unknown Volunteer'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {reg.check_in_time ? formatDate(reg.check_in_time) : 'Not checked in'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <Link
                ...
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
                    <div className="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                        <div className="mt-3 text-center">
                            <h3 className="text-lg leading-6 font-medium text-gray-900">Update Event Status</h3>
                            <div className="mt-4 px-2">
                                <div className="mb-4">
                                    <label className="block text-gray-700 text-sm font-bold mb-2">
                                        Current Status: {getStatusBadge(event.status)}
                                    </label>
                                    <select
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <button
                                        ...
                                        disabled={updateLoading || !newStatus || newStatus === event.status}
                                    >

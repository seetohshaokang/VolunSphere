...
const AdminVerifications = () => {
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      setVolunteers(prev => prev.filter(vol => vol._id !== selectedVolunteer._id));
      ...
      alert(`NRIC verification ${verificationStatus ? 'approved' : 'rejected'} successfully`);
    } catch (err) {
          ...
          {volunteers.length > 0 ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {volunteers.map((volunteer) => (
                    <tr key={volunteer._id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">{volunteer.user_id?.email || 'Unknown'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">{volunteer.phone || 'Not provided'}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {formatDate(volunteer.nric_image?.uploaded_at)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          onClick={() => openVerificationModal(volunteer)}
            ...
            <div className="bg-white rounded-lg shadow p-8 text-center">
              ...
              <p className="mt-2 text-gray-500">
                All volunteer NRIC verifications have been processed. Check back later for new submissions.
              </p>
            </div>
          )}
          ...
          {volunteers.length > 0 && (
            <div className="flex justify-between items-center mt-6">
              <div className="text-sm text-gray-700">
                ...
                </span> of <span className="font-medium">{pagination.total}</span> pending verifications
              </div>
      ...
      {showVerificationModal && selectedVolunteer && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg leading-6 font-medium text-gray-900 text-center">Verify NRIC Document</h3>
              <div className="mt-4 px-2">
                <div className="mb-4">
                  ...
                  <div className="text-center mb-4">
                    <p className="font-bold text-lg">{selectedVolunteer.name}</p>
                    <p className="text-gray-600">{selectedVolunteer.user_id?.email || 'Unknown'}</p>
                  </div>
                  ...
                  <div className="flex space-x-2 mb-4">
                    ...
                    <button
                      ...
                      onClick={() => setVerificationStatus(false)}
                    >
                      Reject
                    </button>
                  </div>
                  ...
                  <label className="block text-gray-700 text-sm font-bold mb-2">
                    Reason for {verificationStatus ? 'Approval' : 'Rejection'} (Optional)
                  </label>
                  <textarea
                    ...
                    rows="3"
                    placeholder={verificationStatus ? "Approved - valid NRIC" : "Rejected - document unclear"}
                  ></textarea>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={verificationLoading}
                  >
                    {verificationLoading ? 'Processing...' : verificationStatus ? 'Approve' : 'Reject'}
                  </button>
                </div>
              </div>
            </div>

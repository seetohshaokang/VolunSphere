...
function EventDetail() {
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 404) {
					...
					throw new Error(
						errorData.message || "Failed to load event details"
					);
				}
			}
	...
	const cancelRegistration = async () => {
		...
		try {
			console.log(
				"Attempting to cancel registration for event:",
				eventId
			);
	...
	const submitReport = async () => {
		...
		try {
			const response = await Api.reportEvent(
				eventId,
				reportReason,
				reportDetails
			);
			...
			if (response.ok) {
				setSuccessMessage(
					"Event reported successfully. Our team will review your report."
				);
									...
									{isEventCompleted() ? (
										...
										<div className="flex gap-2">
											<button
												...
												className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 px-4 rounded-md text-base font-medium transition-colors"
											>
												Cancel Signup
											</button>
											<button
												onClick={handleReportClick}
												className="bg-red-100 hover:bg-red-200 text-red-600 p-3 rounded-md transition-colors"
							...
							{event.max_volunteers > 0 && (
								<div className="mt-5 text-center text-gray-600">
									<p className="mb-2">
										...
										of{" "}
										<strong>{event.max_volunteers}</strong>{" "}
										spots left
									</p>
				...
				<DialogContent className="sm:max-w-md bg-white">
					<DialogHeader>
						...
						<DialogDescription>
							{isSignedUp ? (
								<p>
									Are you sure you want to cancel your signup
									for this event? This action cannot be
									undone.
								</p>
							) : (
								<>
									<p className="mb-2">
										You are about to sign up as a volunteer
										for:
									</p>
									...
									{event.is_recurring && (
										<p className="font-medium text-yellow-600 mt-2 mb-2">
											<strong>Important:</strong> This is
											a recurring event. You can only sign
											up for one instance of a recurring
											event. If you sign up for this
											event, you won't be able to sign up
											for other instances.
										</p>
									)}
								</>
					...
					<DialogFooter className="sm:justify-end">
						...
						<Button
							...
							onClick={
								isSignedUp ? cancelRegistration : confirmSignup
							}
							className={
								isSignedUp ? "" : "border-2 border-black"
							}
						>
							{isSignedUp
					...
					<DialogHeader>
						...
						<DialogDescription>
							<p className="mb-4">
								...
								relevant certification documents to complete the
								verification process.
							</p>
							<p className="text-sm text-gray-600">
								Verification helps establish trust with
								volunteers and ensures all organizers meet our
								community standards.
							</p>
						</DialogDescription>
					</DialogHeader>
					...
					<DialogHeader>
						...
						<DialogDescription>
							<p className="mb-4">
								...
								event. Our team will review your report and take
								appropriate action.
							</p>
						</DialogDescription>
					</DialogHeader>
					...
					<div className="space-y-4">
						<div className="space-y-2">
							...
							>
								...
								<option value="Event doesn't exist">
									Event doesn't exist
								</option>

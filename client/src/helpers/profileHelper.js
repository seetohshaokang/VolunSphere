 ...
 */
export const getProfileImageUrl = (user, profilePictureUrl, timestamp = Date.now()) => {
    ...
    if (profilePictureUrl.startsWith("/") || profilePictureUrl.includes(".")) {
        ...
        if (profilePictureUrl.startsWith("/uploads/") || profilePictureUrl.includes("profile-")) {
            return `http://localhost:8000${profilePictureUrl.startsWith("/") ? "" : "/uploads/profiles/"}${profilePictureUrl}?t=${timestamp}`;
        }
        return `${profilePictureUrl}?t=${timestamp}`;
    }

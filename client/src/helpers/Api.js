...
const Api = {
	...
	cancelEventRegistration(eventId) {
		console.log(
			`Sending DELETE request to ${SERVER_PREFIX}/events/${eventId}/signup`
		);
		return fetch(`${SERVER_PREFIX}/events/${eventId}/signup`, {
			headers: {
				Accept: "application/json",
			...
			.catch((error) => {
				console.error(
					"Network error in cancelEventRegistration:",
					error
				);
				throw error;
			});
	...
	requestPasswordReset(email) {
		return fetch(`${this.SERVER_PREFIX}/auth/reset-password`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},

...
import { getEventImageUrl, getRemainingSpots } from "../../helpers/eventHelper";
...
function EventCard({ event, isOrganizerView = false }) {
  ...
  const isEventRecurring = () => {
    // Debugging for this specific event
    if (event.name === "recurring event") {
      console.log(
        ...
        event.is_recurring,
        "type =",
        typeof event.is_recurring
      );
    }

    // Check if it's the specific "recurring event" by name
    if (event.name === "recurring event") {
      return true;
    }

    // Direct comparison with true
      ...
      <div className="p-4">
        <div className="flex flex-col mb-2">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            ...
            {event.status && (
              <Badge
                className={`${getStatusBadgeStyles(event.status)} font-medium`}
              >

	...
	const fetchVolunteers = async () => {
		...
		try {
			...
			if (data && data.registrations) {
				...
				toast.info("No volunteers registered for this event yet.");
				setVolunteers([]);
			}
		} catch (err) {
			console.error("Error fetching volunteers:", err);
			toast.error("Failed to load volunteer data. Please try again");
							...
							<TableBody>
								{volunteers.map((registration) => (
									<TableRow key={registration._id}>
										...
										<TableCell>
											{registration.volunteer_id?.phone ||
												"N/A"}
										</TableCell>
										<TableCell>
											{new Date(
					...
					<DialogContent className="sm:max-w-md">
						...
						<div className="py-4">
							<p className="mb-4">
								...
								<strong>
									{selectedVolunteer.volunteer_id?.name}
								</strong>{" "}
								from this event?
							</p>
